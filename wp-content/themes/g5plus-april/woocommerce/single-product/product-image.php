...
$carousel_mode = g5Theme()->options()->get_product_single_main_image_carousel_enable();
?>
<div id="single-product-image" class="single-product-image-inner">
    <?php
        do_action('g5plus_april_after_single_product_image');
    ?>
    <div class="single-product-image-main-wrap">
        <?php if('on' !== $carousel_mode): ?>
            ...
            <div class="single-product-image-main nav-center nav-icon owl-carousel manual">
                ...
                if(count($product_images) > 0){
                    foreach($product_images as $key => $value) {
                        ...
                        $image_thumb = wp_get_attachment_image_src($image_id);
                        $image_thumb_link = '';
                        if (is_array($image_thumb) && sizeof($image_thumb) > 0 ) {
                            $image_thumb_link = $image_thumb['0'];
                        }
                        $image       	= wp_get_attachment_image( $image_id, apply_filters( 'single_product_large_thumbnail_size', array('500', '650') ), array(
                            'title'	=> $image_title,
                            'alt'	=> $image_title
                        ) );
    ...
    <?php
    $product_has_gallery_thumb = apply_filters('g5plus_product_has_gallery_thumb', (count($product_images) > 1));
    if(is_array($product_images) && $product_has_gallery_thumb) : ?>
